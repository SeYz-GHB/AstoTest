    ...
    <section className="min-h-screen mt-2">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 w-full md:w-[90%] mx-auto justify-items-center px-2 md:px-0 ">
        {data.map((eachData) => (
          <div
            key={eachData.id}
            className="md:border shadow hover:shadow-md flex flex-col p-2 justify-between w-full max-w-xs hover:scale-105 transition duration-100 ease-in-out cursor-pointer hover:shadow-green-300 rounded-[15px]" onClick={() =>navigate(`/keyboards/${eachData.id}`)}
          >
            ...
            <div className="flex flex-col md:p-2">
              ...
              <div className="flex justify-between w-full">
                ...
                <button className=" text-xs md:text-[20px] font-bold text-white hover:bg-black cursor-pointer duration-200 ease-in-out border p-1 px-4 md:px-5 bg-green-600 rounded-[10px]">
                  <p>Add   </p>
                </button>
              </div>
            </div>
